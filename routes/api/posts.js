...
router.get('/', (req, res) => {
    ...
    .sort({date: -1})
    //json the posts to user
    .then(posts => res.json(posts))
    
    .catch(err => res.status(404).json({nopostsfound: 'No posts available'}));
...
router.get('/:id', (req, res) => {
    ...
    .catch(err => res.status(404).json({nopostfound: 'No post available with that ID'}));
    ...
    .then(profile => {
        Post.findById(req.params.id)
            .then(post => {
                ...
                //get remove index by cycling through likes (indexOf returns index of user id as specified)
